...
//  defaultTaskHandle = osThreadCreate(osThread(defaultTask), NULL);
  ...
  /* add threads, ... */
  status = xTaskCreate(Task1, "Led", 128, NULL, 1, &Task1_Handle);
  configASSERT(status == pdPASS);

  NextTask = Task1_Handle;

  status = xTaskCreate(Task2, "Led2", 128, NULL, 2, &Task2_Handle);
    configASSERT(status == pdPASS);
...
//		btn_status = HAL_GPIO_ReadPin(GPIOC,BUTTON_EXTI13_Pin);
//		if(btn_status==0)
//		{
//			xTaskNotifyFromISR(NextTask,0,eNoAction,0);
